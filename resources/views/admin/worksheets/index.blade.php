...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive p-0">
                ...
                <tbody>
                    @forelse ($worksheets as $worksheet)
                        <tr>
                            ...
                            <td class="py-2">{{\Carbon\Carbon::parse($worksheet->created_at)->format('d/m/Y H:m:s') }}</td>
                            <td class="py-2">{{\Carbon\Carbon::parse($worksheet->updated_at)->format('d/m/Y H:m:s') }}</td>
                            @can('down-document')
                            <td class="py-2 px-2">
                                <a href="{{ Storage::url($worksheet->file) }}" target="blank" class="btn btn-block btn-default btn-xs" title="Baixar arquivo">
                            ...
                            @can('edit-document')
                            <td class="py-2 px-2">
                                <a href="{{route('admin.worksheets.edit',['id' => $worksheet->id])}}" class="btn btn-block btn-info btn-xs" title="Alterar registro">
            ...
            @if ($worksheets)
                <div class="px-2 pt-3">
                    {{ $worksheets->links('pagination::bootstrap-5') }}
                </div>
            @endif
